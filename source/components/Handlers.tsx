...
export const toggleSelection = (
	...
	if (currentItem.isDirectory) {
		...
		const allItemsInFolderAreSelected = itemsInFolder.every((item: Item) =>
		...
		if (allItemsInFolderAreSelected) {
			...
			setSelectedItems(
				selectedItems.filter(
					(item: Item) => !itemsInFolder.find((i: Item) => i.id === item.id),
				),
			);
		} else {
		...
		if (selectedItems.find((item: Item) => item.id === currentItem.id)) {
			...
			setSelectedItems(
				selectedItems.filter((item: Item) => item.id !== currentItem.id),
			);
		} else {
