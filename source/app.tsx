...
const App: FC = () => {
    ...
    const copyContentsOfFilesAndFolders = () => {
        const { content, fileCount } = outputXml(selectedItems);
        clipboard.writeSync(content);
        setMessage(
            <Text color="white">✨ Successfully copied <Text color="cyan">{fileCount}</Text> file{fileCount > 1 && "s"} to clipboard</Text>
        );
        setTimeout(() => {
            process.exit(0);
        }, 300);
