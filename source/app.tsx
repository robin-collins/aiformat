...
const App: FC = () => {
	...
	const {items, setItems, gitignoreRules, isGitignorePresent} =
		useFileHandlers();
	...
	return (
		<Box flexDirection="column" marginTop={2} marginBottom={2}>
			<Box flexDirection="column" marginBottom={1}>
				<Text>
					<Text color="blue">aiformat</Text> version:{' '}
					<Text color="yellow">{AIFORMAT_VERSION}</Text>,{' '}
					<Text color="green">generateOutput</Text> version:{' '}
					<Text color="yellow">{GENERATE_OUTPUT_VERSION}</Text>.
				</Text>
				{isGitignorePresent ? (
					<Text>
						<Text color="redBright">.gitignore present:</Text>{' '}
						<Text color="yellow">[{gitignoreRules.join(', ')}]</Text>
					</Text>
				) : null}
				<Text>
					default excluded folders:[
					<Text color="yellow">{EXCLUDED_FOLDERS.join(', ')}</Text>]
				</Text>
			</Box>
			...
			<Box marginTop={1}>{message && <Text>{message}</Text>}</Box>
		</Box>
	);
};
